from flask import Flask, render_template
from flask_cors import CORS

app = Flask(__name__)
app.secret_key = 'secret123'
CORS(app, supports_credentials=True)
app.config.from_object('config.Config')

# URLs de los servicios desde la configuración
USERS_SERVICE_URL = app.config.get('USERS_SERVICE_URL', 'http://localhost:5002')
PRODUCTS_SERVICE_URL = app.config.get('PRODUCTS_SERVICE_URL', 'http://localhost:5003')
ORDERS_SERVICE_URL = app.config.get('ORDERS_SERVICE_URL', 'http://localhost:5004')

# Ruta para renderizar el template index.html
@app.route('/')
def index():
    return render_template('index.html', 
                         users_service_url=USERS_SERVICE_URL,
                         products_service_url=PRODUCTS_SERVICE_URL,
                         orders_service_url=ORDERS_SERVICE_URL)

@app.route('/dashboard')
def dashboard():
    return render_template('dashboard.html',
                         users_service_url=USERS_SERVICE_URL,
                         products_service_url=PRODUCTS_SERVICE_URL,
                         orders_service_url=ORDERS_SERVICE_URL)

# Ruta para renderizar el template users.html
@app.route('/users')
def users():
    return render_template('users.html',
                         users_service_url=USERS_SERVICE_URL)

# Ruta para renderizar el template products.html
@app.route('/products')
def products():
    return render_template('products.html',
                         products_service_url=PRODUCTS_SERVICE_URL)

# Ruta para renderizar el template orders.html
@app.route('/orders')
def orders():
    return render_template('orders.html',
                         orders_service_url=ORDERS_SERVICE_URL,
                         users_service_url=USERS_SERVICE_URL,
                         products_service_url=PRODUCTS_SERVICE_URL)

@app.route('/editUser/<string:id>')
def edit_user(id):
    print("id recibido",id)
    return render_template('editUser.html', id=id,
                         users_service_url=USERS_SERVICE_URL)

@app.route('/editProduct/<string:id>')
def edit_product(id):
    print("id recibido",id)
    return render_template('editProduct.html', id=id,
                         products_service_url=PRODUCTS_SERVICE_URL)

@app.route('/editOrder/<string:id>')
def edit_order(id):
    print("id recibido",id)
    return render_template('editOrder.html', id=id,
                         orders_service_url=ORDERS_SERVICE_URL)

if __name__ == '__main__':
    app.run()
